...
async function uploadOneAsset(
  ...
  const { filename, contentType, bytes } = await resolveAssetBytes(asset);
...
function buildServer(client: MdlogClient): McpServer {
      ...
      try {
        ...
        if (assets && assets.length > 0) {
          ...
          for (const asset of assets) {
            const assetKey = await uploadOneAsset(client, norm.path, asset);
...
async function main(): Promise<void> {
  ...
  process.stderr.write(
    `md-log-mcp ready (backend: ${config.apiBaseUrl})\n`,
  );
}
